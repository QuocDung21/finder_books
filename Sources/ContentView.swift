...
enum MainAppTab: String, CaseIterable, Identifiable {
    case splitter = "Tách Sách"
    case library = "Thư Viện & Gom Nhóm"
    
    var id: String { rawValue }
    var icon: String {
        switch self {
        case .splitter: return "scissors"
...
struct ContentView: View {
    ...
    @State private var selectedTab: MainAppTab = .splitter
    @State private var isDropTargeted: Bool = false
    @State private var isLogsExpanded: Bool = true
    
    var body: some View {
        Group {
            if selectedTab == .splitter {
                splitterWorkspaceView
            } else {
                BookLibraryView { bookURL in
                    vm.loadPDF(from: bookURL)
            ...
            ToolbarItemGroup(placement: .primaryAction) {
                if selectedTab == .splitter {
                    Button {
                        ...
                        Label("Mở PDF", systemImage: "doc.badge.plus")
                    }
                    .help("Chọn file PDF (⌘O)")
                    .keyboardShortcut("o", modifiers: .command)
                    
                    if vm.outputFolderURL != nil {
                        Button {
                            vm.revealInFinder()
                        } label: {
                            Label("Mở Thư Mục", systemImage: "folder")
                        }
                        .help("Mở thư mục lưu trong Finder (⇧⌘O)")
        ...
        .alert(isPresented: $vm.showAlert) {
            if vm.alertShowCancel {
                return Alert(
                    title: Text(vm.alertTitle),
                    message: Text(vm.alertMessage),
                    primaryButton: .default(Text(vm.alertConfirmButtonTitle)) {
                        vm.alertConfirmAction?()
                    },
                ...
                return Alert(
                    title: Text(vm.alertTitle),
                    message: Text(vm.alertMessage),
                    dismissButton: .default(Text(vm.alertConfirmButtonTitle))
                )
            }
        }
    ...
    // MARK: - Splitter Workspace View
    private var splitterWorkspaceView: some View {
        HSplitView {
    ...
    private var documentHeroView: some View {
        Group {
            if let thumb = vm.thumbnailImage {
                // File Loaded View with Thumbnail
                HStack(spacing: 16) {
                    Image(nsImage: thumb)
                        .resizable()
                ...
                VStack(spacing: 12) {
                    ...
                    VStack(spacing: 4) {
                        ...
                        Text("Hoặc bấm nút bên dưới để chọn sách từ máy tính")
                            .font(.system(size: 12))
                            .foregroundColor(.secondary)
                    }
